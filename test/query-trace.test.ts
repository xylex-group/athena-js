...
import type { AthenaQueryTraceEvent } from "../src/client.ts";
...
function getCurrentLine(): number {
	const frame = new Error("line capture").stack?.split("\n")[2]?.trim();
...
function assertUserCallsite(
	trace: AthenaQueryTraceEvent,
	expectedLine: number,
) {
	assert.ok(trace.callsite);
	assert.equal(trace.callsite?.fileName, "query-trace.test.ts");
	assert.match(
		trace.callsite?.filePath?.replace(/\\/g, "/") ?? "",
...
test("traceQueries=true logs select traces with sql, outcome, and callsite", async () => {
...
test("traceQueries custom logger receives upsert/rpc/query events", async () => {
	...
	try {
		...
		const upsertMutation = client
			...
			.upsert(
				{ id: 1, name: "Frodo" },
				{ onConflict: "id", updateBody: { name: "Frodo" } },
			);
		const upsertLine = getCurrentLine() + 1;
		await upsertMutation.select("id");

		const rpcQuery = client
			.rpc("list_users", { active: true })
			.eq("active", true);
...
test("traceQueries includes debug ASTs when debugAst is enabled", async () => {
	const captured: AthenaQueryTraceEvent[] = [];
	const originalFetch = globalThis.fetch;
	globalThis.fetch = async () => createMockResponse([{ id: 1 }], 200);
