...
e2eTest(
	"e2e: insert, filter, and delete rows in test table (athena_logging client)",
	async () => {
		...
		try {
			const insertResult = await client
				...
				.insert(payload)
				.single(
					"id,test_bool,test_text,test_number,test_json,test_time,test_uuid",
				);
			...
			const insertedData = insertResult.data as
				| { id?: number }
				| { id?: number }[]
				| null;
			...
			assert.equal(
				fetchResult.error,
				null,
				`fetch failed: ${fetchResult.error?.message ?? ""}`,
			);
			assert.ok(
				Array.isArray(fetchResult.data),
				"fetch should return an array",
			);
			assert.equal(
				fetchResult.data?.length,
				1,
				"fetch should return one matching row",
			);
