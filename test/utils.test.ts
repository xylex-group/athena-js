...
function withMockDocument(
	...
	try {
		...
		// Some runtimes seal globalThis.document; fall back to assignment.
		try {
			(globalThis as { document?: unknown }).document = documentMock;
		} catch {
			// Skip mock if the environment forbids overriding document.
			run([]);
			return;
		}
	}
...
function withMockCrypto(fill: (bytes: Uint8Array) => void, run: () => void) {
...
test("trimTrailingSlashes removes one or more trailing slash characters", () => {
	assert.equal(
		trimTrailingSlashes("https://example.com/"),
		"https://example.com",
	);
	assert.equal(
		trimTrailingSlashes("https://example.com///"),
...
test("readTrimmedString, asNumber, and asStringArray coerce utility payloads safely", () => {
	...
	assert.deepEqual(asStringArray([" a ", "b", 3, "", "   ", "c"]), [
		"a",
		"b",
		"c",
	]);
...
test("sqlText wraps values in a deterministic dollar-quoted literal", () => {
	withMockCrypto(
		...
		() => {
			assert.equal(
				sqlText("hello world"),
				"$s000000000000$hello world$s000000000000$",
			);
		},
	);
});
...
test("sqlText preserves payload contents when the initial tag collides with the value", () => {
	withMockCrypto(
		...
		() => {
			...
			assert.equal(
				literal,
				"$s000000000000_$before $s000000000000$ after$s000000000000_$",
			);
...
test("sqlNullableText returns NULL for nullish values and quotes strings", () => {
	...
	withMockCrypto(
		...
		() => {
			assert.equal(
				sqlNullableText("value"),
				"$s000000000000$value$s000000000000$",
			);
		},
	);
});
...
test("sqlJsonbLiteral serializes payloads and casts to jsonb", () => {
	withMockCrypto(
		...
		() => {
			const literal = sqlJsonbLiteral({ count: 2, ok: true });
			assert.match(literal, /^\$s000000000000\$/);
			assert.match(literal, /\$s000000000000\$::jsonb$/);
			const json = literal.slice(
				"$s000000000000$".length,
				-"$s000000000000$::jsonb".length,
			);
			assert.deepEqual(JSON.parse(json), { count: 2, ok: true });
		},
	);
});
...
test("clearAuthCookies clears athena/better-auth cookie prefixes across domain candidates", () => {
	withMockDocument(
		"athena-auth.session=abc; foo=bar; __Secure-better-auth.session=def",
		(writes) => {
...
test("clearAuthCookies avoids domain attributes for local hostnames", () => {
	withMockDocument(
		"__Secure-athena-auth.session=abc; other=value",
		(writes) => {
...
test("proxyRequestHeaders removes host and applies forwarded headers from request URL", () => {
	const request = new Request(
		"https://preview.example.com:8443/auth/callback?flow=pkce",
		{
			headers: {
