...
test("delete propagates range/limit bounds into payload", async () => {
  const { calls, restore } = mockFetch();
  try {
...
test("update propagates limit bounds into payload", async () => {
  const { calls, restore } = mockFetch();
  try {
    await client
      .from("events")
...
test("upsert forwards head/count to payload", async () => {
  ...
  try {
    await client
      .from("characters")
      .upsert({ id: 1, name: "Aragorn" }, { count: "planned", head: true })
      .select("id");
    const payload = JSON.parse(calls[0].init?.body as string);
...
test("maybeSingle allows zero rows", async () => {
  const { restore } = mockFetch();
  try {
...
test("upsert supports onConflict array", async () => {
  ...
  try {
    await client
      .from("characters")
      .upsert({ id: 1 }, { onConflict: ["id", "name"] })
      .select("id");
    const payload = JSON.parse(calls[0].init?.body as string);
...
test("canonical chained range filters work: gte + lt after select", async () => {
  ...
  try {
    await client
      ...
      .gte("population", 1000)
      .lt("population", 10_000);

    const payload = JSON.parse(calls[0].init?.body as string);
    assert.equal(payload.table_name, "cities");
    assert.equal(payload.columns, "name,country_id");
    assert.deepEqual(payload.conditions, [
      { column: "population", operator: "gte", value: 1000 },
      { column: "population", operator: "lt", value: 10_000 },
    ]);
  } finally {
    restore();
  }
...
test("findMany appends where conditions and overrides existing order/limit state", async () => {
  ...
  try {
    ...
    // Chained `.eq("status")` first, then where keys in object insertion order.
    assert.deepEqual(payload.conditions, [
      {
        column: "status",
        eq_column: "status",
...
test("P0: relation some() filters parents without Gateway", async () => {
  const { calls, restore } = mockFetch();
  ...
  try {
    const { error } = await astClient.from("orchestral_sections").findMany({
      select: { name: true },
...
test("findManyAst preserves uuid-aware equality fallback inside where objects", async () => {
  const { calls, restore } = mockFetch();
  const client = createClient({
    db: { url: "https://athena-db.com" },
    findManyAst: true,
...
test("findManyAst falls back when relation nodes target an explicit schema", async () => {
  const { calls, restore } = mockFetch();
  const client = createClient({
    db: { url: "https://athena-db.com" },
    findManyAst: true,
...
test("findManyAst falls back when chained filters already exist", async () => {
  const { calls, restore } = mockFetch();
  const client = createClient({
    db: { url: "https://athena-db.com" },
    findManyAst: true,
...
test("table options deduplicate matching schema for insert/upsert/update/delete", async () => {
  ...
  try {
    ...
    const tableNames = calls.map(
      (call) => JSON.parse(call.init?.body as string).table_name
    );
...
test("rpc order, limit, and offset map to payload", async () => {
  ...
  try {
    await client
      ...
      .select("id");
    const payload = JSON.parse(calls[0].init?.body as string);
    assert.deepEqual(payload.order, { ascending: false, column: "created_at" });
...
test("rpc count exact is sent and surfaced on result", async () => {
  ...
  globalThis.fetch = async (url, init) => {
    calls.push({ init, url: String(url) });
    return new Response(JSON.stringify({ count: 7, data: [{ id: 1 }] }), {
      status: 200,
    });
  };
  try {
    const result = await client
      .rpc<{ id: number }>("list_characters", undefined, { count: "exact" })
      .select("id");
    const payload = JSON.parse(calls[0].init?.body as string);
    assert.equal(payload.count, "exact");
...
test("rpc single and maybeSingle return first row", async () => {
  ...
  try {
    const single = await client
      .rpc<{ id: number; name: string }>("list_characters")
      .single("id,name");
...
test("rpc throws when function name is blank", () => {
  assert.throws(() => client.rpc("   "), /rpc requires a function name/);
});
...
test("rpc select-level options override constructor options", async () => {
  ...
  try {
    await client
      ...
      .select("id", { count: "exact", schema: "private" });
...
test("rpc order defaults ascending to true when not provided", async () => {
  ...
  try {
    ...
    const payload = JSON.parse(calls[0].init?.body as string);
    assert.deepEqual(payload.order, { ascending: true, column: "created_at" });
  } finally {
    restore();
  }
});
...
test("rpc result with count keeps count after single()", async () => {
  const original = globalThis.fetch;
  globalThis.fetch = async () =>
    new Response(JSON.stringify({ count: 3, data: [{ id: 1 }] }), {
      status: 200,
    });
  try {
    const result = await client
      .rpc<{ id: number }>("list_characters")
      .single("id");
...
test("query calls /gateway/query with { query } payload", async () => {
  ...
  try {
    ...
    assert.ok(calls[0].url.endsWith("/gateway/query"));
    const payload = JSON.parse(calls[0].init?.body as string);
    // 3.7+: additive operation/expectedShape for Dragunov; Athena 4 ignores unknown fields.
    assert.deepEqual(payload, {
      expectedShape: "rows",
      operation: "select",
...
test("retryReads retries select after a retryable network failure", async () => {
  let attempts = 0;
  const original = globalThis.fetch;
  globalThis.fetch = async () => {
    attempts += 1;
  ...
  try {
    ...
    const result = await retryingClient
      .from("characters")
      .select("id")
      .limit(1);
...
test("retryReads retries query after a retryable network failure", async () => {
  let attempts = 0;
  const original = globalThis.fetch;
  globalThis.fetch = async () => {
    attempts += 1;
...
test("retryReads retries findManyAst reads after a retryable network failure", async () => {
  let attempts = 0;
  const original = globalThis.fetch;
  globalThis.fetch = async () => {
    attempts += 1;
...
test("retryReads does not retry writes by default", async () => {
  let attempts = 0;
  const original = globalThis.fetch;
  globalThis.fetch = async () => {
    attempts += 1;
