...
test("select builds fetch payload with Athena-style filters", async () => {
  ...
  globalThis.fetch = async (url, init) => {
    received.push({ init, url: String(url) });
  ...
  try {
    ...
    assert.deepEqual(payload.conditions, [
      { column: "level", operator: "gt", value: 5 },
    ]);
  } finally {
...
test("x-athena-client header is sent for routing when client option is set", async () => {
  ...
  try {
    ...
    const reqHeaders = (received[0].init?.headers ?? {}) as Record<
      string,
      string
    >;
    const clientHeader =
    ...
    assert.equal(
      clientHeader,
      "xylex_cloud",
      "X-Athena-Client header must be sent for routing"
    );
  } finally {
    globalThis.fetch = originalFetch;
  }
...
test("insert mutation supports select() and returning rows", async () => {
  ...
  globalThis.fetch = async (url, init) => {
    ...
    return new Response(JSON.stringify([{ id: 1, name: "Frodo" }]), {
      status: 201,
    });
  };
  try {
    ...
    assert(
      (result.data as Array<{ name?: string }> | null)?.[0]?.name === "Frodo"
    );
...
test("insert success message on 2xx does not surface as error", async () => {
  ...
  try {
    ...
    assert.equal((result.data as Array<{ id?: number }> | null)?.[0]?.id, 123);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("eqCast serializes cast hints on fetch payload when query fallback is disabled", async () => {
  ...
  globalThis.fetch = async (url, init) => {
    calls.push({ init, url: String(url) });
    return new Response(JSON.stringify({ count: 0, data: [], status: 200 }), {
      status: 200,
    });
  };
  try {
