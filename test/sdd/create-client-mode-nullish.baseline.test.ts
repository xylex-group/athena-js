/**
 * Baseline characterization: createClient mode/prefer nullish handling as shipped.
 * Documents current applyExecutionMode semantics (mode != null / prefer != null).
...
test("baseline: omitted mode/prefer is not a mode hint (storage-only R2 materializes)", async () => {
...
test("baseline: explicit null mode/prefer is not a mode hint (storage-only R2 materializes)", async () => {
	const r2 = createSddMockR2();
	const client = createClient({
		mode: null,
		prefer: null,
	...
	// null != null is false → hasModeHint stays false without D1/env → early return
	assert.equal(client.capabilities.mode, "cloudflare-edge");
	assert.equal(client.capabilities.storage.local, true);
	assert.equal(client.capabilities.storage.objects, true);
	assert.equal(client.capabilities.db.local, false);
...
test("baseline: non-null mode/prefer still resolve (prefer gateway drops local db)", () => {
