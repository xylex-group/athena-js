...
- Live Athena server baseline used for verification on this page: `3.12.3`, confirmed from `GET https://mirror2.athena-cluster.com` on 2026-06-04.
...
`athena-js` exposes the stable `createClient({ findManyAst: true, ... })` option for gateways that explicitly support direct AST bodies, but the default SDK transport still stays on the compiled `columns`/`conditions` contract described above.
