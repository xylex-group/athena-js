...
## Auto-create / update config

Prefer a committed config file? Use `init` or let `generate` keep it current:

```bash
# Create a modern athena.config.ts (direct or gateway from env)
athena-js init
athena-js init --mode gateway

# Intelligent update only: auto-fill provider.schemas from live discovery
# Does not rewrite custom output/naming/features when schemas already match
athena-js init --dry-run
```

`athena-js generate` also:

- discovers non-system schemas that contain tables (direct + gateway)
- expands the effective schema list for introspection
- creates/updates `athena.config.ts` surgically when needed
- skips the write when schemas are already current (safe for typecheck/CI)

Disable config writes or discovery when you want pure env-only CI:
...
- if you want flat `src/lib/athena/generated/models/*.ts` files instead of schema-scoped defaults, set `output.targets.model = "src/lib/athena/generated/models/{model_kebab}.ts"` or `ATHENA_GENERATOR_MODEL_TARGET=src/lib/athena/generated/models/{model_kebab}.ts`
...
```ts
import { __athena_schema_meta, registry } from "./src/lib/athena/generated/registry";
