...
For **membership patterns** (session vs arbitrary `userId`, gateway `member`
table): [Organization membership](../organization-membership.md).
...
- `auth.organization.list()` -> `GET /organization/list` — orgs visible to the **current session user** (not `userId`)
...
- `auth.organization.requirePermission()` -> session + org permission guard helper
