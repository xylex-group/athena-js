...
- `key` -> `X-Athena-Key`
- `context.cookie` → original cookie plus derived `X-Athena-Auth-Session-Token`
- `context.bearerToken` → original bearer plus mirrored `X-Athena-Auth-Bearer-Token`
...
`storage.prefixPath` is used by the high-level `athena.storage.file.*` helpers. It is prepended to upload keys and list prefixes. Templates support values from call options, per-call `vars`, config `vars`, and env values:
...
Pass S3/R2 credentials through `storage.directUpload` to send file bytes directly from the SDK runtime to object storage:
...
This keeps the existing `file.upload(...)` input, progress callbacks, and `AthenaStorageFileUploadResult` contract. Athena still creates the managed-file metadata; only the file-byte `PUT` bypasses Athena and is signed locally with Web Crypto. The configured bucket must match the bucket returned by the metadata endpoint. The storage endpoint must allow browser CORS for the signed `PUT` headers.

Direct-upload credentials are available to the runtime, so use short-lived, least-privilege credentials or a narrowly scoped browser credential. Direct upload does not make metadata, list, download, delete, or other storage methods serverless: those methods continue to use their existing Athena routes.
...
| --- | --- | --- | --- | --- |
| `listStorageCatalogs()` | `GET /storage/catalogs` | Raw JSON | `{ data: S3CatalogItem[] }` | List registered S3/R2 (S3-compatible) catalogs for the selected Athena client. |
| `createStorageCatalog(input)` | `POST /storage/catalogs` | Raw JSON | `S3CatalogItem` | Register a catalog and its initial credential. Supports `provider: "s3"` / `"r2"` and optional `account_id` for Cloudflare R2. |
...
## Cloudflare R2 catalogs

Storage catalog providers are `s3` and `r2`. The server also accepts `aws` /
`aws_s3` as S3 aliases and `cloudflare_r2` / `cloudflare-r2` as R2 aliases.
Unknown labels are handled as S3-compatible unless the endpoint or metadata
identifies R2.

Create a catalog with R2 using the same SDK method as S3. Prefer explicit `provider` and either a full R2 endpoint or `account_id`:

```ts
await athena.storage.createStorageCatalog({
  name: "prod-r2",
  provider: "r2",
  account_id: process.env.CF_ACCOUNT_ID!,
  bucket: "media",
  access_key_id: process.env.R2_ACCESS_KEY_ID!, // 32 chars, no hyphens
  secret_key: process.env.R2_SECRET_ACCESS_KEY!,
})
```

Or pass a full endpoint:
...
Managed file/object methods do not need R2-specific APIs — they resolve the catalog and use the server’s S3-compatible client.

### R2 S3 API limits (object level)

Cloudflare R2 implements a **subset** of the S3 object API. Safe building blocks:

- Head / Get / Put / Delete / ListObjectsV2  
- Multipart (create, upload part, complete, abort, list parts)  
- CopyObject without ACL/tagging/KMS  
- SSE-C on several ops  

**Not on R2:** object tagging, ACL grants, object lock, SSE-KMS, request-payer,
expected-bucket-owner. Prefer ListObjectsV2. On UploadPart, reusing a part number
**replaces** the previous part (a failed re-upload loses the original).
...
## Database backups (`storage.backup`)

Admin backup/restore/jobs/schedules are available under `client.storage.backup.*` (same routes as Studio `/admin/backups*`). Archives land in the **server** object-storage profile (`ATHENA_BACKUP_S3_*`), which may be AWS S3 or Cloudflare R2.

```ts
const page = await athena.storage.backup.list({ client_name: "athena_logging" })
await athena.storage.backup.create({ client_name: "athena_logging", label: "daily" })
const jobs = await athena.storage.backup.jobs.list({ limit: 50 })
```

See `docs/storage/r2-and-backup-impact-report.md` for migration notes.
...
Register a global observer through `storage.onError`, or pass a per-call observer:
