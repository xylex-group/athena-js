...
These examples place Athena DB calls directly inside `useQuery` and `useMutation` (`createAthenaBrowserClient` from `@xylex-group/athena/next/client` + `athena.from(...).select()/insert()/eq()`), without any TanStack/React Query dependency.

For request-scoped Next server construction, see `examples/next/adapters.ts` and `@xylex-group/athena/next/server`.
