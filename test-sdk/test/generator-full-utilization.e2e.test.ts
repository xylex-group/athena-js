...
function createMinimalPgCatalogMock() {
  return async (sqlText: string) => {
    ...
    throw new Error(
      `Unexpected SQL in direct provider example test: ${sqlText.slice(0, 80)}...`
    );
  };
}
...
test("generator full-utilization example: dry-run and write mode cover placeholders, naming, and metadata", async () => {
  ...
  try {
    const dryRun = await runGeneratorDryRunWithSnapshot(
      workspace,
      config,
      snapshot
    );
    ...
    const writeRun = await runGeneratorWriteWithSnapshot(
      workspace,
      config,
      snapshot
    );
    ...
    // table-builder default: reserved column name + relation metadata via Object.assign
    assert.equal(modelFile.includes("table: string()"), true);
    assert.equal(modelFile.includes("relations:"), true);

    const athenaModelPath = join(
      workspace.cwd,
      "src",
      "generated",
      "app-db",
    ...
    // nullable jsonb maps to json<Record<string, unknown>>().optional()
    assert.equal(
      athenaModel.includes(
...
test("generator full-utilization example: programmatic rendering handles feature toggles", () => {
  ...
  const usersModel = artifacts.files.find(
    (file) =>
      file.path.endsWith("/public/users.model.ts") ||
      file.path.endsWith("\\public\\users.model.ts")
  );
  assert(usersModel);
...
test("generator full-utilization example: gateway-only provider path runs through /gateway/query", async () => {
  ...
  try {
    const gatewayConfig = createGatewayOnlyGeneratorConfig(
      "https://athena-db.com",
      "secret"
    );
