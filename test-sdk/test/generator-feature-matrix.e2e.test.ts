...
function createMinimalPgCatalogMock() {
  return async (sqlText: string) => {
    ...
    if (sqlText.includes("WHERE con.contype = 'p'")) {
      return {
        rows: [{ columns: ["id"], schema_name: "public", table_name: "users" }],
      };
    }
    ...
    throw new Error(
      `Unexpected SQL in direct provider matrix test: ${sqlText.slice(0, 80)}...`
    );
  };
}

function withMockedPoolQuery(
  mockImpl: (sql: string) => Promise<{ rows: QueryResultRow[] }>
) {
  ...
  (Pool.prototype.query as unknown as (
    sql: string
  ) => Promise<{ rows: QueryResultRow[] }>) = mockImpl;
...
function createGatewayFetchMock(options: GatewayMockOptions = {}) {
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (
      ...
      payload.query.includes(options.failOnPattern)
    ) {
      return new Response(
        JSON.stringify({
          error: options.failureMessage ?? "Gateway query failed",
        }),
...
test("generator matrix: direct config builder uses postgres direct mode", () => {
  const config = createDirectGeneratorConfig(
    "postgres://postgres:postgres@127.0.0.1:5432/app_db"
  );
...
test("generator matrix: gateway config builder uses postgres gateway mode", () => {
  const config = createGatewayOnlyGeneratorConfig(
    "https://athena-db.com",
    "secret"
  );
  assert.equal(config.provider.kind, "postgres");
...
test("generator matrix: full snapshot includes multi-schema and reserved identifiers", () => {
  ...
  assert.equal(
    snapshot.schemas.public.tables.users.columns.table.name,
    "table"
  );
  assert.equal(
    snapshot.schemas.public.tables.profiles.columns.mood.typeKind,
    "enum"
  );
...
test("generator matrix: writeGeneratorConfigFile writes athena.config.ts at workspace root", async () => {
  ...
  try {
    ...
    rmSync(workspace.cwd, { force: true, recursive: true });
  }
});

test("generator matrix: ensureWorkspace is idempotent and does not throw", async () => {
  const workspace = createWorkspace(
    "athena-generator-matrix-ensure-workspace-"
  );
  try {
...
test("generator matrix: loadResolvedExampleConfig throws when config file is missing", async () => {
  ...
  try {
    await assert.rejects(
      async () => loadResolvedExampleConfig(workspace),
      /was not found/
    );
  } finally {
    rmSync(workspace.cwd, { force: true, recursive: true });
...
test("generator matrix: renderArtifactsFromExampleSnapshot includes expected model path", () => {
  ...
  const artifacts = renderArtifactsFromExampleSnapshot(config, snapshot);
  assert.equal(
    artifacts.files.some(
      (file) => file.path === "src/generated/app-db/public/users.model.ts"
    ),
    true
  );
});
...
test("generator matrix: rendered model content keeps reserved column keys", () => {
  ...
  const artifacts = renderArtifactsFromExampleSnapshot(config, snapshot);
  const modelFile = artifacts.files.find(
    (file) => file.path === "src/generated/app-db/public/users.model.ts"
  );
  assert(modelFile);
...
test("generator matrix: custom placeholder namespace path mapping is supported", () => {
  ...
  config.output.targets.model =
    "src/generated/{namespace}/{model_kebab}.model.ts";
...
test("generator matrix: unknown placeholder tokens raise explicit errors", () => {
  ...
  config.output.targets.model =
    "src/generated/{missing_token}/{model_kebab}.model.ts";
  assert.throws(
    () => renderArtifactsFromExampleSnapshot(config, snapshot),
...
test("generator matrix: collectTypeMappingShowcase includes expected mapped primitives", () => {
  ...
  assert.equal(
    showcase.some((entry) => entry.mappedType === "string"),
    true
  );
});
...
test("generator matrix: collectTypeMappingShowcase maps array dimensions", () => {
  const showcase = collectTypeMappingShowcase();
  const arrayCase = showcase.find(
    (entry) => entry.source.arrayDimensions === 1
  );
...
test("generator matrix: runGeneratorDryRunWithSnapshot does not write files", async () => {
  ...
  try {
    ...
    const result = await runGeneratorDryRunWithSnapshot(
      workspace,
      config,
      snapshot
    );
...
test("generator matrix: runGeneratorWriteWithSnapshot writes registry and model files", async () => {
  ...
  try {
    ...
    const result = await runGeneratorWriteWithSnapshot(
      workspace,
      config,
      snapshot
    );
...
test("generator matrix: direct provider inspect works from pg_url with mocked pg catalog", async () => {
  ...
  try {
    const snapshot = await runDirectProviderInspect(
      "postgres://postgres:postgres@127.0.0.1:5432/app_db"
    );
...
test("generator matrix: gateway provider inspect executes catalog queries over /gateway/query", async () => {
  ...
  try {
    ...
    assert.equal(
      gatewayMock.calls.every((call) => call.url.endsWith("/gateway/query")),
      true
    );
    assert.equal(
      gatewayMock.calls.every((call) => call.method === "POST"),
      true
    );
  } finally {
    gatewayMock.restore();
...
test("generator matrix: gateway provider queries include schema array literal filters", async () => {
  ...
  try {
    ...
    assert.equal(
      gatewayMock.calls.some((call) =>
        call.query.includes("ARRAY['public', 'athena']::text[]")
      ),
      true
    );
  } finally {
...
test("generator matrix: gateway provider sends backend routing header for postgresql", async () => {
  ...
  try {
    await runGatewayProviderInspect("https://athena-db.com", "secret");
    assert.equal(
      gatewayMock.calls.some(
        (call) => call.headers["X-Backend-Type"] === "postgresql"
      ),
      true
    );
  } finally {
