...
const program = ts.createProgram(
  sourceFiles.map((file) => path.join(cwd, file)),
  {
...
function sanitizeDocsText(text) {
  return text
    .replace(
      /\{@link\s+([^}\s]+)(?:\s+[^}]*)?\}/g,
      (_, target) => `\`${target}\``
    ...
    .replace(/\{@code\s+([^}]+)\}/g, (_, code) => `\`${code.trim()}\``)
    .replace(/\s+/g, " ")
    .trim();
}
...
function collectInterfaceMethods(
  ...
  if (!iface) {
    throw new Error(
      `Interface not found: ${interfaceName} in ${sourceFilePath}`
    );
  }
  ...
  function walkType(type, currentPath) {
    for (const prop of checker.getPropertiesOfType(type)) {
      ...
      const nestedProps = checker
        ...
        .filter((nested) => {
          const nestedDecl =
            nested.valueDeclaration ?? nested.declarations?.[0];
...
function collectClassMethods(sourceFilePath, className, prefix) {
  ...
  const results = [];
  const members = klass.members.filter(
    (member) =>
  ...
  const constructorMethods = klass.members.filter((member) =>
...
function exampleForPath(pathName, minArgs) {
  ...
  if (pathName.startsWith("athena.from.")) {
    ...
    if (method === "select") {
      return 'await athena.from("users").select("id,name")';
    }
    if (method === "insert") {
      return 'await athena.from("users").insert({ name: "Ada" }).select()';
    }
    if (method === "upsert") {
      return 'await athena.from("users").upsert({ id: "u_1", name: "Ada" }).select()';
    }
    if (method === "update") {
      return 'await athena.from("users").eq("id", "u_1").update({ name: "Ada" }).select()';
    }
    if (method === "delete") {
      return 'await athena.from("users").eq("id", "u_1").delete()';
    }
    if (method === "single" || method === "maybeSingle") {
      return `await athena.from("users").eq("id", "u_1").${method}()`;
    }
    if (method === "reset") {
      return 'athena.from("users").eq("id", "u_1").reset()';
    }
    if (
      [
        "eq",
  ...
  if (pathName.startsWith("athena.rpc.")) {
    const method = pathName.split(".").at(-1);
    if (method === "select") {
      return 'await athena.rpc("list_users").select()';
    }
    if (method === "single" || method === "maybeSingle") {
      return `await athena.rpc("list_users").${method}()`;
    }
  ...
  if (pathName.startsWith("athena.db.")) {
    const method = pathName.split(".").at(-1);
    if (method === "from") {
      return 'athena.db.from("users").select()';
    }
    if (method === "select") {
      return 'await athena.db.select("users", "id,name")';
    }
    if (method === "insert") {
      return 'await athena.db.insert("users", { name: "Ada" }).select()';
    }
    if (method === "upsert") {
      return 'await athena.db.upsert("users", { id: "u_1", name: "Ada" }).select()';
    }
    if (method === "update") {
      return 'await athena.db.update("users", { name: "Ada" }).eq("id", "u_1").select()';
    }
    if (method === "delete") {
      return 'await athena.db.delete("users", { resourceId: "u_1" }).select()';
    }
    if (method === "rpc") {
      return 'await athena.db.rpc("list_users").select()';
    }
    if (method === "query") {
      return 'await athena.db.query("select * from users")';
    }
  ...
  if (pathName.startsWith("athena.chat.")) {
    const method = pathName.split(".").at(-1);
    if (pathName === "athena.chat.room.list") {
      return "await athena.chat.room.list({ limit: 20 })";
    }
    if (pathName === "athena.chat.room.create") {
      return 'await athena.chat.room.create({ slug: "engineering", name: "Engineering" })';
    }
    if (pathName === "athena.chat.room.get") {
      return 'await athena.chat.room.get("room_1")';
    }
    if (pathName === "athena.chat.room.update") {
      return 'await athena.chat.room.update("room_1", { name: "Engineering" })';
    }
    if (pathName === "athena.chat.room.archive") {
      return 'await athena.chat.room.archive("room_1")';
    }
    if (pathName === "athena.chat.room.readCursor.upTo") {
      return 'await athena.chat.room.readCursor.upTo("room_1", { seq: 42 })';
    }
    if (pathName === "athena.chat.room.member.list") {
      return 'await athena.chat.room.member.list("room_1")';
    }
    ...
    if (pathName === "athena.chat.realtime.info") {
      return "await athena.chat.realtime.info()";
    }
  ...
  if (pathName.startsWith("athena.storage.")) {
    const _method = pathName.split(".").at(-1);
    if (pathName === "athena.storage.listStorageCatalogs") {
      return "await athena.storage.listStorageCatalogs()";
    }
    if (pathName === "athena.storage.listStorageCredentials") {
      return "await athena.storage.listStorageCredentials()";
    }
    if (pathName === "athena.storage.getStorageFile") {
      return 'await athena.storage.getStorageFile("file_1")';
    }
    ...
    if (pathName === "athena.storage.file.get") {
      return 'await athena.storage.file.get("file_1")';
    }
  ...
  if (pathName.startsWith("AthenaQueryClient.")) {
    ...
    if (method === "constructor") {
      return 'const queryClient = new AthenaQueryClient({ cache: { mode: "memory" } })';
    }
  ...
  if (pathName.startsWith("cookies.")) {
    ...
    return minArgs === 0
      ? `${pathName}()`
      : `${pathName}(${method.includes("Cookie") ? "/* cookie inputs */" : "/* ... */"})`;
  }

  if (pathName.startsWith("react.")) {
    const method = pathName.split(".").at(-1);
    if (method === "useQuery") {
      return 'const query = useQuery({ queryKey: ["users"], queryFn: () => athena.from("users").select() })';
    }
    if (method === "useMutation") {
      return 'const mutation = useMutation({ mutationFn: (v) => athena.from("users").insert(v).select() })';
    }
    if (method === "useSession") {
      return "const session = useSession(athena)";
    }
  ...
  if (pathName.startsWith("root.")) {
    ...
    if (method === "createClient") {
      return 'const athena = createClient({ url: "https://...", key: "api_key" })';
    }
    if (method === "runSchemaGenerator") {
      return 'await runSchemaGenerator({ configPath: "./athena.config.ts" })';
    }
    if (method === "defineModel") {
      return 'const model = defineModel<{ id: string }>({ meta: { primaryKey: ["id"], nullable: { id: false } } })';
    }
    if (method === "defineSchema") {
      return 'const schema = defineSchema({ users: defineModel<{ id: string }>({ meta: { primaryKey: ["id"], nullable: { id: false } } }) })';
    }
    if (method === "defineDatabase") {
      return "const db = defineDatabase({ public: defineSchema({}) })";
    }
    if (method === "defineRegistry") {
      return "const registry = defineRegistry({ primary: defineDatabase({ public: defineSchema({}) }) })";
    }
    if (method === "createModelFormAdapter") {
      return "const adapter = createModelFormAdapter(model)";
    }
    if (method === "toModelFormDefaults") {
      return "const defaults = toModelFormDefaults(model, row)";
    }
    if (method === "toModelPayload") {
      return "const payload = toModelPayload(model, values)";
    }
    if (method === "AthenaClient") {
      return 'const athena: AthenaClient = createClient({ url: "https://...", key: "api_key" })';
    }
  ...
  const rows = entries
    ...
    .map((entry) => {
      const signature = sanitizeSignature(entry.signature)
        .replace(/\|/g, "\\|")
        .replace(/\n/g, " ");
      const docs = sanitizeDocsText(
        [entry.docs, entry.deprecated ? `Deprecated: ${entry.deprecated}` : ""]
          .filter(Boolean)
          .join(" ")
      ).replace(/\|/g, "\\|");
      const example = exampleForPath(entry.path, entry.minArgs).replace(
        /\|/g,
        "\\|"
      );
...
const runtimeClient = collectInterfaceMethods(
  "src/v3-client.ts",
  "AthenaClient",
  "athena",
  { recursive: false }
);
const runtimeTableBuilder = collectInterfaceMethods(
  "src/client.ts",
  "TableQueryBuilder",
  "athena.from",
  { recursive: false }
);
const runtimeSelectChain = collectInterfaceMethods(
  "src/client.ts",
  "SelectChain",
  "athena.from.select",
  { recursive: false }
);
const runtimeUpdateChain = collectInterfaceMethods(
  "src/client.ts",
  "UpdateChain",
  "athena.from.update",
  { recursive: false }
);
const runtimeRpcBuilder = collectInterfaceMethods(
  "src/client.ts",
  "RpcQueryBuilder",
  "athena.rpc",
  { recursive: false }
);
const runtimeDb = collectInterfaceMethods(
  "src/db/module.ts",
  "AthenaDbModule",
  "athena.db",
  { recursive: false }
);
const runtimeChat = collectInterfaceMethods(
  "src/chat/types.ts",
  "AthenaChatModule",
  "athena.chat"
);
const runtimeStorage = collectInterfaceMethods(
  "src/storage/module.ts",
  "AthenaStorageModule",
  "athena.storage"
);

const authBindings = collectInterfaceMethods(
  "src/auth/types.ts",
  "AthenaAuthBindings",
  "athena.auth",
  { recursive: true }
...
const reactQueryClientClass = collectClassMethods(
  "src/react/query-client.ts",
  "AthenaQueryClient",
  "AthenaQueryClient"
);
const reactExports = [
  ...
  ...collectExportedFunctionsFromEntry(
    "src/react/use-athena-session-client.ts",
    "react"
  ),
];

const nextClientExports = collectExportedFunctionsFromEntry(
  "src/next/client.ts",
  "next.client"
);
const nextServerExports = collectExportedFunctionsFromEntry(
  "src/next/server.ts",
  "next.server"
);

const cookieExports = collectExportedFunctionsFromEntry(
  "src/cookies/index.ts",
  "cookies"
);
const utilsExports = collectExportedFunctionsFromEntry(
  "src/utils/index.ts",
  "utils"
);
...
const totalMethods = sections.reduce(
...
// Site MDX is dual-published via apps/docs/scripts/sync-athena-js-docs.mts
// (frontmatter, generated banner, link rewrites). Do not write the site tree here.
console.log(`Generated ${outputPath}`);
console.log(
  "Site copy: run `pnpm run docs:site:sync` (or apps/docs docs:sync-athena-js) after regenerating."
);
